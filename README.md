# EasyMeeting

EasyMeeting 是一个基于 Electron + Vue 3 的桌面端会议演示项目，当前阶段聚焦“纯前端可运行、可演示、可迭代”的会议业务闭环。

<a id="toc"></a>

> 文档目标：用最少的阅读成本了解项目能力、启动方式与后续规划。

## 文档导航

### 项目概览

- [最新开发进展](#latest-progress)
- [技术栈](#tech-stack)
- [功能展示（骨架）](#feature-overview)
- [界面截图](#screenshots)

### 使用指南

- [快速开始](#quick-start)
- [目录结构（核心）](#project-structure)
- [UI 设计与体验策略](#ui-strategy)
- [说明](#notes)

### 开发规划

- [下一阶段开发计划（会议项目路线图）](#roadmap)
- [会议项目路线图文档](docs/meeting-project-roadmap.md)

---

<a id="latest-progress"></a>

## 最新开发进展

### 已完成能力（当前版本）

#### 1) 桌面壳与工作区切换

- 自定义窗口标题栏（最小化、最大化/还原、关闭）。
- 登录区 / 会议区工作模式切换联动。

#### 2) 登录与鉴权

- 登录/注册模式切换与表单校验（邮箱、昵称、密码、验证码）。
- 本地验证码生成与刷新（SVG）。
- 路由守卫：未登录无法进入会议模块。

#### 3) 会议管理（列表 + 详情）

- 会议中心看板：概览指标、今日安排、冲突预警、时间线视图。
- 会议列表：检索、状态筛选、排序、倒计时、卡片/时间线双视图。
- 会议操作：新建、编辑、复制、删除、复制房间号。
- 会议详情：基础信息、议程、参会人、备注、状态进度、风险提示、会前检查清单、关键里程碑。
- 冲突检测：会议室占用冲突 + 参会人时间冲突（本地计算）。
- 会前提醒：手动提醒与自动提醒（10 分钟/1 分钟）。

#### 3.1) 会议创建体验升级

- 分区式编辑弹窗：核心信息 / 权限策略 / 参会人与议程。
- 右侧实时预览：时间、时长、参会人、议程与会议策略。
- 完成度进度条：引导信息填写完整度。

#### 4) 会议房间（纯前端演示）

- 会前检查：
  - 摄像头/麦克风预览（`getUserMedia`）。
  - 设备切换、昵称设置、设备状态提示。
  - 会前设置本地持久化（昵称、设备、开关状态）。
- 房间内：
  - 主舞台 + 参会者侧栏 + 聊天侧栏。
  - 麦克风/摄像头/举手/共享屏幕（本地模拟）控制。
  - 参会者状态动态展示（麦克风、摄像头、举手、共享）。
  - 本地聊天 + 远端消息模拟 + 表情选择器。
  - 快捷键支持（`M` 麦克风、`V` 摄像头、`R` 举手）。

#### 5) 代码结构演进（已完成）

- 会议房间从单文件拆分为模块化结构：
  - 视图装配层：`src/renderer/src/views/Meeting/room.vue`
  - 房间编排逻辑：`src/renderer/src/views/Meeting/composables/useMeetingRoom.js`
  - 媒体子模块：`src/renderer/src/views/Meeting/composables/room/useRoomMedia.js`
  - 远端模拟子模块：`src/renderer/src/views/Meeting/composables/room/useRoomSimulation.js`
  - 样式模块：`src/renderer/src/views/Meeting/styles/room.scss`

#### 6) 本地持久化与回退

- Electron 环境：`electron-store`。
- Web 环境：自动回退 `localStorage`。

[返回目录](#toc)

---

<a id="tech-stack"></a>

## 技术栈

- Electron
- Vue 3
- Element Plus
- Electron-Vite
- Vue Router
- TailwindCSS + PostCSS
- ESLint + Prettier

[返回目录](#toc)

---

<a id="feature-overview"></a>

## 功能展示（骨架）

> 本章节用于对外展示已有能力的“精简版说明”，与“最新开发进展”的详尽清单互补。

- 登录与鉴权：登录/注册切换、表单校验、本地验证码、路由守卫。
- 会议中心看板：核心指标、今日安排、冲突预警、时间线视图。
- 会议列表与详情：检索筛选、排序、倒计时、状态进度、风险提示与清单。
- 会议创建体验：分区式编辑、右侧实时预览、完成度进度条。
- 会前检查：设备预览、设备切换、昵称与状态持久化。
- 会议房间演示：主舞台/参会者/聊天三栏、举手/共享/设备状态联动、快捷键控制。

### 架构图

![架构图](docs/images/architecture.png)

[返回目录](#toc)

---

<a id="screenshots"></a>

## 界面截图

### 冲突预警一眼定位风险

![会议中心看板](docs/images/dashboard.png)

### 任务入口高效可筛选

![会议列表](docs/images/meeting-list.png)
![会议列表2](docs/images/meeting-list2.png)

### 详情视图聚合关键信息

![会议详情](docs/images/meeting-detail.png)

### 创建流程更直观可控

![会议创建弹窗](docs/images/meeting-create.png)

### 会中主界面可视化协作

![会议房间主界面](docs/images/meeting-room.png)

### 设备准备更可见更可信

![会前检查](docs/images/precheck.png)

### 互动沟通更实时可控

![聊天与互动](docs/images/chat.png)

### 登录入口清晰可用

![登录页](docs/images/login.png)

[返回目录](#toc)

---

<a id="quick-start"></a>

## 快速开始

### 推荐执行顺序

1. 安装依赖
2. 启动开发环境
3. 执行代码检查
4. 运行测试
5. 构建产物

### 安装依赖

```bash
npm install
```

### 启动开发环境

```bash
npm run dev
```

### 代码检查

```bash
npm run lint
```

### 运行测试

```bash
npm test
```

### 构建

```bash
npm run build
```

### 平台打包

```bash
npm run build:win
npm run build:mac
npm run build:linux
```

[返回目录](#toc)

---

<a id="project-structure"></a>

## 目录结构（核心）

```text
.
├── README.md
├── docs/
│   ├── meeting-project-roadmap.md
│   ├── images/
│   └── ...
└── src/
    ├── main/
    └── renderer/src/
        ├── components/
        ├── mock/
        ├── router/
        ├── utils/
        └── views/
            ├── Login/
            └── Meeting/
                ├── components/
                ├── composables/
                │   ├── room/
                │   │   ├── useRoomMedia.js
                │   │   └── useRoomSimulation.js
                │   └── useMeetingRoom.js
                ├── styles/
                │   └── room.scss
                ├── list.vue
                ├── detail.vue
                └── room.vue
```

[返回目录](#toc)

---

<a id="ui-strategy"></a>

## UI 设计与体验策略

- 会议中心作为“运营看板”呈现：指标 + 冲突预警 + 时间线。
- 详情页采用主副栏结构：主栏承载内容，副栏承载状态/风险/清单。
- 表单强调“即时反馈”：输入即预览，提升可解释性与完成度可视化。

[返回目录](#toc)

---

<a id="roadmap"></a>

## 下一阶段开发计划（会议项目路线图）

路线图已迁移至独立文档，便于持续迭代维护与版本管理：  
[查看会议项目路线图（docs/meeting-project-roadmap.md）](docs/meeting-project-roadmap.md)

[返回目录](#toc)

---

<a id="notes"></a>

## 说明

- 当前版本定位为纯前端演示，不依赖后端接口。
- 若演进到多人实时音视频，需要引入信令服务与 STUN/TURN。

[返回目录](#toc)
